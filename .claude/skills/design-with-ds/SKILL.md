...
4. **Précharge les familles de police du DS** (`preloadFonts()` du prelude, familles lues dans la foundation typographique) en **première instruction** de tout appel `use_figma` qui instancie un composant ou écrit du texte. Une fonte arrive aussi **avec une instance importée** : sans ce préchargement, `appendChild` lève « unloaded font » avant tout accès au texte, dès le premier composant posé.
5. **Réutilise les scripts** au lieu de ré-écrire l'API à la main → dossier [`scripts/`](scripts/) (voir ci-dessous).
...
| Fichier | Usage |
|---|---|
| [`scripts/_prelude.js`](scripts/_prelude.js) | Bloc de helpers à **coller en tête** d'un `use_figma`, puis à appeler. C'est le mode d'emploi quotidien. Deux blocs à coller séparément, le prelude étant repayé à chaque appel : **noyau** (`preloadFonts`, `applyColor`, `applyText`, `setText`, `bindSpacing`, `autoLayout`, `applyMode`, `instantiate`) et **texte long & overrides d'instance** (`wrapText`, `instanceTexts`, `instanceChild`). |
| [`scripts/factor-local-component.js`](scripts/factor-local-component.js) | **Factorisation d'un assemblage répété** en composant local : convertit l'occurrence de référence en composant, range la définition hors du flux et remplace les autres occurrences par des instances (même parent, index, position, sizing). La correction canonique des `repeatedAssemblies` de l'audit. **Une violation `repeatedAssemblies` passe par ce script, jamais par une factorisation improvisée** : il mémorise parent, index et comportement de sizing **avant** `createComponentFromNode` (qui remplace le nœud). Le refaire à la main repose l'instance au mauvais index ou lui fait perdre son sizing — un correctif de plus pour un problème déjà résolu. |
